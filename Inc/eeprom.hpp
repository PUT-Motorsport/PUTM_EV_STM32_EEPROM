...
public:
  ...
  public:
    Variable(Eeprom &new_eeprom, ADDRESS_TYPE new_address, SIZE_TYPE init_value)
        : eeprom{new_eeprom}, address{static_cast<uint16_t>(new_address) + 1} {
    ...
    [[nodiscard]] std::optional<SIZE_TYPE> read() {

      if (!eeprom.is_initialized || state == VariableState::NOT_INITIALIZED)
        return std::nullopt;
    ...
    bool write(SIZE_TYPE new_value) {
      ...
      if constexpr (sizeof(SIZE_TYPE) == 1) {
        status =
            EE_WriteVariable8bits(address, std::bit_cast<uint8_t>(new_value));
      } else if constexpr (sizeof(SIZE_TYPE) == 2) {
        status =
            EE_WriteVariable16bits(address, std::bit_cast<uint16_t>(new_value));
      } else if constexpr (sizeof(SIZE_TYPE) == 4) {
        status =
            EE_WriteVariable32bits(address, std::bit_cast<uint32_t>(new_value));
      }
